...
describe('MockEscrow contract with upgradable arbitrator', async () => {
  ...
  beforeEach('Setup contracts', async () => {
    ...
    await arbitrator0.deployed()
    // Make appeals go to the same arbitrator
    await arbitrator0.changeArbitrator(arbitrator0.address)

    contractArtifact = await hre.artifacts.readArtifact('MockEscrowUpgradable')
    ...
    await contract.changeArbitrator(arbitrator.address, arbitratorExtraData)
  })
  ...
  describe('Appeals', () => {
    ...
    it('Should handle appeal fees correctly while emitting the correct events', async () => {
      ...
      expect(txPromise1)
        ...
        .withArgs(transactionId, BigNumber.from(0), TransactionParty.Receiver)
      ...
      expect(txPromise2)
        .to.emit(contract, 'HasPaidAppealFee')
        .withArgs(transactionId, BigNumber.from(0), TransactionParty.Sender)
