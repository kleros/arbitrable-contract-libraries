...
contract MockEscrow is IArbitrable, IEvidence, BinaryAppealable {
    ...
    function withdrawFeesAndRewards(address payable _beneficiary, uint256 _transactionID, Transaction calldata _transaction, uint256 _round) public onlyValidTransactionCD(_transactionID, _transaction) {
        ...
        BinaryAppealable.Round[] storage rounds = roundsByTransactionID[_transactionID];
        uint256 reward = _withdrawFeesAndRewards(rounds, _beneficiary, _round, uint256(transactionDispute.ruling));
     ...
     */
    function withdrawRoundBatch(address payable _beneficiary, uint256 _transactionID, Transaction calldata _transaction, uint256 _cursor, uint256 _count) public onlyValidTransactionCD(_transactionID, _transaction) {
        require(_transaction.status == Status.Resolved, "The transaction must be resolved.");
        TransactionDispute storage transactionDispute = disputeIDtoTransactionDispute[_transaction.disputeID];
        require(transactionDispute.transactionID == _transactionID, "Undisputed transaction");

        BinaryAppealable.Round[] storage rounds = roundsByTransactionID[_transactionID];
        uint256 reward = _withdrawRoundBatch(rounds, _beneficiary, _cursor, _count, uint256(transactionDispute.ruling));
        _beneficiary.send(reward); // It is the user responsibility to accept ETH.
    }
    ...
    function rule(uint256 _disputeID, uint256 _ruling) public override {
        ...
        if (round.sideFunded == BinaryAppealable.BaseParty(uint256(Party.Sender)))
            transactionDispute.ruling = uint8(Party.Sender);
        else if (round.sideFunded == BinaryAppealable.BaseParty(uint256(Party.Receiver)))
    ...
    function amountWithdrawable(uint256 _transactionID, Transaction calldata _transaction, address _beneficiary) public view onlyValidTransactionCD(_transactionID, _transaction) returns (uint256 total) {
        ...
        BinaryAppealable.Round[] storage rounds = roundsByTransactionID[_transactionID];
        total = _amountWithdrawable(rounds, _beneficiary, uint256(transactionDispute.ruling));
    }
